...
import jp.androidgroup.nyartoolkit.markersystem.android.NyARAndroidMarkerSystem;
 ...
 *
 */
public class CubeRenderer extends MarkerSystemAbstractRenderer
{
	...
	public CubeRenderer()
	{
		// マーカーパターン読み込みはここに設置することも可能。
		// その場合はクラス変数も追加してください
		// 動的に変える場合は、別途メソッドを追加したほうが無難です。
	...
	/* (non-Javadoc)
	 * @see jp.androidgroup.nyartoolkit.markersystem.renderer.MarkerSystemAbstractRenderer#configureARScene()
	 * 
	 * ここでマーカーのパターンを登録します。
	 * 
	 */
	...
	{
		// MarkerSystem の インスタンス取得
		NyARAndroidMarkerSystem _nyARMS = NyARAndroidMarkerSystemFactory.getInstance().getMarkerSystem();
		
		// パターンファイル名定義
		String[] _marker = {"AR/Marker/hiro.pat", "AR/Marker/kanji.pat"};
		
		// マーカーID管理用領域作成
		// このIDを用いて、検出結果などを取得する
		markerIds = new int[_marker.length];
		
		// パターンファイル読み込み
		try {
			for (int i=0; i<_marker.length; i++) {
				InputStream _markerIS = assetMng.open(_marker[i]);
				markerIds[i] = _nyARMS.addARMarker(_markerIS, 16, 25, 80);
				_markerIS = null;
			}
			
			return true;
		} catch (IOException e) {
			Log.d(TAG, "ARToolKit marker file is not found.");
		} catch (NyARException e) {
			Log.d(TAG, "Add Marker pattern error.");
		}
...
//		gl.glLoadIdentity();
		gl.glLoadMatrixf(NyARAndroidMarkerSystemFactory.getInstance().getMarkerSystem().getGlProjectionMatrixf(), 0);
		...
		// 識別したマーカー上にオブジェクトを描く処理
		// markerIds の数だけ回して、識別していたら描画するという処理
		for (int i=0; i<markerIds.length; i++) {
			if (NyARAndroidMarkerSystemFactory.getInstance().getMarkerSystem().isExistMarker(markerIds[i])) {
				Log.d(TAG, "draw visible");
				gl.glMatrixMode(GL10.GL_MODELVIEW);
				gl.glLoadIdentity();
				gl.glPushMatrix();
				gl.glLoadMatrixf(NyARAndroidMarkerSystemFactory.getInstance().getMarkerSystem().getGlMarkerMatrixf(markerIds[i]), 0);
